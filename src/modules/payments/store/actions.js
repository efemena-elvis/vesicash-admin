...
const routes = {
  ...
  pending_transactions_count:'/list-wallet-transactions?count=yes&status=pending'
};
...
export default {
  fetchAllTransactions: async ({ commit }, { page, payload }) => {
    ...
    const response = await $api.use('admin').push(routes.transactions(page), { payload });
  ...
  fetchPendingTransactions: async ({ commit }, query) => {
    ...
    const response = await $api.use('admin').fetch(routes.pending_transactions(query));
  ...
  fetchPendingTransactionsCount: async ({ commit }) => {
    ...
    const response = await $api.use('admin').fetch(routes.pending_transactions_count);
