...
export default {
  ...
  methods: {
    ...mapActions({
      updateUserDollarAccount: "users/updateUserDollarAccount",
    }),

    updateSavedDetails(details) {
      this.form = {
        account_last_name: details?.account_name?.split(" ")[1],
        account_first_name: details?.account_name?.split(" ")[0],
    ...
    async updateNewDollarAccount() {
      ...
      try {
        const response = await this.updateUserDollarAccount(
          this.getNewDollarAccountDetails
        );
