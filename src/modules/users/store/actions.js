...
const routes = {
  ...
  update_user_access: "/user-details",
};
...
export default {
  ...
  fetchUserProfile: async ({ commit }, account_id) => {
    const response = await $api
      .use("admin")
      .fetch(routes.user_profile(account_id));
  ...
  fetchAllFxTransactions: async ({ commit }, payload) => {
    const response = await $api
      .use("admin")
      .push(routes.user_fx_txn, { payload });
  ...
  fetchWalletWithdrawals: async (_, { account_id, page }) => {
    return await $api
      .use("admin")
      .fetch(routes.wallet_withdrawals(account_id, page));
  },

  fetchWalletFundings: async (_, { account_id, page }) => {
    return await $api
      .use("admin")
      .fetch(routes.wallet_fundings(account_id, page));
  },
  ...
  fetchVesicashModerators: async ({ commit }) => {
    ...
    if (response?.code === 200)
      commit("SAVE_VESICASH_MODERATORS", response?.data);
    return response;
  },
  ...
  fetchModeratorsAccessTypes: async ({ commit }) => {
    const response = await $api
      .use("admin")
      .fetch(routes.moderator_access_types);
    if (response?.code === 200)
      commit("SAVE_MODERATORS_ACCESS_TYPES", response?.data);
    return response;
  },
  ...
  approveOrDeclineTransaction: async (_, payload) => {
    return await $api
      .use("admin")
      .push(routes.approve_decline_txn, { payload });
  },
  ...
  updateUserDollarAccount: async (_, payload) => {
    return await $api
      .use("admin")
      .push(routes.update_dollar_account, { payload });
  },
  ...
  deleteModerator: async (_, payload) => {
    return await $api
      .use("admin")
      .remove(routes.delete_moderator, { payload, resolve: true });
  },
  ...
  async updateUserAccess(
    { commit, state },
    { account_id, authStatus, authField, payload }
  ) {
    let response = await $api
      .use("admin", "v2")
      .push(routes.update_user_access + `/${account_id}`, { payload });
    if (response.code === 200) {
      commit("SAVE_USER_PROFILE", {
        ...state.user_profile,
        user: {
          ...state?.user_profile?.user,
          [authField]: authStatus,
        },
      });
    }
    return response;
