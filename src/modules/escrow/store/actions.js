...
const routes = {
    escrow_transactions: (query) => `/business/transactions/list?${query}`,
};

export default {
    fetchEscrowTransactions: async({ commit }, query) => {
        const response = await $api.use('admin').fetch(routes.escrow_transactions(decodeURIComponent(query)));
