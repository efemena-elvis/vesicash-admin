<template>
  <div class="activity-card rounded-16 border-grey-100 neutral-10-bg">
    <!-- USER ACTIVITIES -->
    <div class="activities">
      <template v-for="(activity, index) in activities">
        <div class="activity" :key="index">
          <!-- DESCRIPTION TEXT -->
          <div class="description-text grey-900 tertiary-2-text mgb-6">{{ activity.description }}</div>

          <!-- TIMESTAMP TEXT -->
          <div
            class="timestamp-text grey-600 tertiary-3-text"
          >{{ activity.created_at | formatDate($date) }}</div>
        </div>
      </template>
    </div>
  </div>
