...
export default {
  ...
  computed: {
    ...
    charges() {
      if (!this.getCharges?.wallet_withdrawal) return [];
      const first_key = Object.keys(this.getCharges?.wallet_withdrawal)?.[0];
      return this.getCharges?.wallet_withdrawal?.[
    ...
    tabs() {
      return this.getCharges?.wallet_withdrawal
        ? Object.keys(this.getCharges?.wallet_withdrawal).map(
            (currency, index) => {
  ...
  async mounted() {
    if (!this.getCharges?.wallet_withdrawal) this.fetchAllCharges();
  },
