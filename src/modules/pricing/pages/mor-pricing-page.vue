...
export default {
  ...
  computed: {
    ...
    charges() {
      if (!this.getCharges?.mor_transaction) return [];
      const first_key = Object.keys(this.getCharges?.mor_transaction)?.[0];
      return this.getCharges?.mor_transaction?.[
    ...
    tabs() {
      return this.getCharges?.mor_transaction
        ? Object.keys(this.getCharges?.mor_transaction).map(
            (currency, index) => {
  ...
  async mounted() {
    if (!this.getCharges?.mor_transaction) this.fetchAllCharges();
  },
  ...
  methods: {
    ...
    async fetchAllCharges() {
      this.loading = true;
      await this.fetchCharges("mor_transaction");
      this.loading = false;
    },
  },
};
