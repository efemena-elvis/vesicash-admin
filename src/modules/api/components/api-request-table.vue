<template>
  <div class="neutral-10-bg api-table">
    ...
    <TableContainer
      table_name="api-tb"
      :table_data="getPaginatedAPIRequests"
      ...
      @goToPage="updatePage"
    >
      <template v-for="(data, index) in getPaginatedAPIRequests">
        <ApiRequestTableRow
          @refresh="fetchRequests(filterQuery)"
          :key="index"
          table_name="api-tb"
          :data="data"
          :index="
...
export default {
  ...
  components: {
    ...
    ApiRequestTableRow: () =>
      import(
        /* webpackChunkName: "api-module" */ "@/modules/api/components/api-request-table-row"
      ),
  },
  ...
  computed: {
    ...mapGetters({
      getAPIRequests: "api/getAPIRequests",
    }),
    ...
    getPaginatedAPIRequests() {
      return this.getAPIRequests?.data;
    },

    getPagination() {
      return {
        current_page: this.getAPIRequests?.current_page,
        per_page: this.getAPIRequests?.per_page,
        last_page: this.getAPIRequests?.last_page,
        from: this.getAPIRequests?.from,
        to: this.getAPIRequests?.to,
        total: this.getAPIRequests?.total,
      };
    },
  },
  ...
  methods: {
    ...mapActions({
      fetchAPIRequests: "api/fetchAPIRequests",
    }),
