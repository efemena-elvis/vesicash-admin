<template>
  <div class="bsn-table-container neutral-10-bg">
    <!-- TABLE CONTAINER -->
    <TableContainer
      table_name="bsn-rate-tb"
...
export default {
  name: "BusinessVerificationTable",
  ...
  components: {
    ...
    VerificationRow: () =>
      import(
        /* webpackChunkName: "mor-module" */ "@/modules/verifications/components/business-verification-row"
      ),
  },
  ...
  computed: {
    ...mapGetters({
      getVerifications: "mor/getBusinessVerifications",
    }),
    ...
    getPaginatedTable() {
      const { per_page } = this;
      const tables = [...this.getVerifications];
  ...
  data() {
    return {
      table_header: ["#", "Account ID", "Business name", "Status", "Action"],
  ...
  mounted() {
    this.fetchAllVerifications(this.filterQuery);
  },

  created() {
    this.$bus.$on("refresh_business", () =>
      this.fetchAllVerifications(this.filterQuery)
    );
  },
  ...
  methods: {
    ...mapActions({
      fetchVerifications: "mor/fetchBusinessVerifications",
    }),
